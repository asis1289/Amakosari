...
export default function EditOffer() {
  ...
  useEffect(() => {
    if (formData) {
      const variantValue = formData.variant || 0;
      console.log('Setting variant to:', variantValue);
      setVariant(variantValue);
      setFormData((prev: any) => ({ ...prev, variant: variantValue }));
  ...
  }, [formData?.title, formData?.discount, formData?.variant]);
  ...
  const fetchOffer = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/offers/${params.id}`, {
      ...
      if (response.ok) {
        ...
        if (data.link && !LINK_OPTIONS.some(opt => opt.value === data.link)) {
          setLinkMode('custom');
          setCustomLink(data.link);
        } else {
          setLinkMode(data.link || '');
        }
      } else {
  ...
  const handleLinkChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setLinkMode(value);
    if (value === 'custom') {
      setFormData((prev: any) => ({ ...prev, link: '' }));
    } else {
      setFormData((prev: any) => ({ ...prev, link: value }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setSuccessMessage(getMessage('success', 'offer', 'updated'));
        setNotificationType('success');
        setShowSuccessNotification(true);
        ...
        const data = await response.json();
        setNotificationType('error');
        setSuccessMessage(data.error || getMessage('error', 'offer', 'update'));
        setShowSuccessNotification(true);
      }
    } catch (err) {
      setNotificationType('error');
      setSuccessMessage(getMessage('error', 'offer', 'update'));
      setShowSuccessNotification(true);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
      ...
      <div className="max-w-2xl mx-auto bg-white rounded-xl shadow-lg p-8 mt-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Where do you want to display this offer?</label>
            <select
              value={linkMode ?? ''}
          ...
          <div>
            ...
            <input
              ...
              value={formData.minimumOrderAmount !== undefined && formData.minimumOrderAmount !== null ? formData.minimumOrderAmount : ''}
              onChange={handleInputChange}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              min="0"
            />
          ...
          <div>
            ...
            {/* Preview uploaded, generated, or previous image */}
            <div className="mt-4 w-64 h-32 flex items-center justify-center bg-gray-50 rounded-lg border" id="offer-image-preview">
              {formData && formData.imageUrl && typeof formData.imageUrl === 'string' && formData.imageUrl.trim() !== '' ? (
                <img
          ...
          <GeneratedOfferImageModal
            ...
            onSave={(newVariant) => {
              ...
              setFormData((prev: any) => ({ ...prev, variant: newVariant }));
